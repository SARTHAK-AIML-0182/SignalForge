  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-5 backdrop-blur-sm space-y-4">
      ...
      <div className="space-y-4">
        {posts.length === 0 ? (
          <div className="text-center py-10 text-slate-500 text-xs font-mono border border-slate-800 rounded-xl bg-slate-950/60 p-6">
            No published intelligence briefings found for this agent session. Trigger a workflow cycle to generate live briefings.
          </div>
        ) : (
          posts.map((post) => {
